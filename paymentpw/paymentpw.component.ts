...
export class PaymentPWComponent implements OnInit {
	...
	now = new Date();
	purchaseCreditIsLoading: boolean = false;
	purchaseCreditError: string = "";
	@Output()
	...
	constructor(private router: Router,
			...
			private paymentProfileHandler: PaymentProfileHandler) {
				...
				this.giveCreditFormGroup = this._formBuilder.group({
					amount: ['', Validators.required],
				});
  ...
  ngOnInit() {
		...
		this.viewing = this.cookieService.get('viewing') == 'true';
	...
	toPay() {
		...
		this.subscribeErrorMsg = "";
		if (this.instanceNameFormGroup.controls.instanceName.value == null ||
			this.instanceNameFormGroup.controls.instanceName.value.length < 1) {
			this.subscribeErrorMsg = "Please enter an instance name.";
			return;
		}
	...
	buyCreditWhenPurchase(){
		...
		{ 	
			...
			if(data2.returnCode == "true"){
				...
				this.purchaseCreditError = "There was an issue with processing your order.";
			}
		});
	}
	...
	buyCredit(){
		this.purchaseCreditIsLoading = true;
		this.purchaseCreditError = "";
		let dataIn: any = {};
		let loginStatus = this.authenticationHandler.isLoggedIn();
		dataIn.paymentByUserId = loginStatus[8];
		...
		{
			let data2 = JSON.parse(data);
			this.purchaseCreditIsLoading = false;
			if(data2.returnCode == "true"){
				this.BuyCreditSuccess = true;
				this.BuyCreditSuccessMsg = "Success.";
				this.isCreditPurchase = false;	
	...
	giveCredit(){
		...
		dataIn.amount = this.giveCreditFormGroup.controls.amount.value;
		// console.log(this.paymentMethods);
		// console.log(dataIn);
		this.paymentProfileHandler.giveAccountCredit(dataIn).then((data)=>
		{ 	console.log("give credit");
			console.log(data);
			let data2 = JSON.parse(data);
			if(data2.returnCode == "true"){
				this.BuyCreditSuccess = true;
				this.BuyCreditSuccessMsg = "Success.";
				this.isCreditGive = false;
				this.updateBalance();	
				this.balanceUpdated.emit();
			}
		});	
	...
	finalizeAndPay(){
		...
		if (response.type == 'Error') { 
			...
			dataIn.fingerprint = Brick.getFingerprint();
